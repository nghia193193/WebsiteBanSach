	...
	<section class="row">
		...
		<div class="col mt-4">
			...
			<div class="card">
				...
				<div class="card-header">Danh sách các quyển sách</div>
				...
				<div class="card-body">
					...
					<c:if test="${!bookPage.hasContent() }">
						<div class="row">
							<div class="col">
								<div class="alert alert-danger">Chưa có cuốn sách nào hãy thêm mới</div>
							</div>
						</div>
					</c:if>
							...
							<c:if test="${bookPage.totalPages > 0 }">
								<nav aria-label="Page navigation">
									<ul class="pagination">
										...
										<a class="page-link"
											href='<c:url value="/admin/stores/${storeId }/books?size=${bookPage.size}&page=${1}"/>'
										...
										<c:forEach items="${pageNumbers }" var="pageNumber">
											<c:if test="${bookPage.totalPages > 1 }">
												...
												<a href='<c:url value="/admin/stores/${storeId }/books?size=${bookPage.size}&page=${pageNumber}"/>' 
												class="page-link">${pageNumber }</a></li>
											</c:if>
										</c:forEach>
											
										<li class="${bookPage.totalPages == bookPage.number+1? 'page-item active' : 'page-item' }">
										<a href='<c:url value="/admin/stores/${storeId }/books?size=${bookPage.size}&page=${bookPage.totalPages}"/>' class="page-link">Last</a>
										</li>
									</ul>
								</nav>
							</c:if>
