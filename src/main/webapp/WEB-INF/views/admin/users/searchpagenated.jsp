			...
			<div class="card">
				...
				<div class="card-body">
					...
					<div class="container-fluid">
							<form class="d-flex" action="/admin/users/searchpagenated">
						...
						<table class="table table-striped table-responsive">
							...
							<tbody>
								<c:forEach items="${userPage.content}" var="user">
										...
										<tr>
											<td><c:if test="${user.avatar != null }">
												<img alt="" src="${user.avatar }" width="70px" class="img-fluid" >
											</c:if>
											...
											<td>${user.username}</td>
											<td>${user.delete ? 'Không hoạt động' : 'Hoạt động'}</td>
											<td>${user.createAt}</td>
											<td>${user.updateAt}</td>
											<td>
												<a
						...
						<div class="col-7">
							...
							<c:if test="${userPage.totalPages > 0 }">
								<nav aria-label="Page navigation">
									<ul class="pagination">
										<li class="${1 == userPage.number+1 ? 'page-item active' : 'page-item' }">
										<a class="page-link"
											href='<c:url value="/admin/users/searchpagenated?name=${name }&size=${userPage.size}&page=${1}"/>'
											tabindex="-1" aria-disabled="true">First</a></li>
										
										<c:forEach items="${pageNumbers }" var="pageNumber">
											<c:if test="${userPage.totalPages > 1 }">
												<li class="${pageNumber == userPage.number+1 ? 'page-item active' : 'page-item' }">
												<a href='<c:url value="/admin/users/searchpagenated?name=${name }&size=${userPage.size}&page=${pageNumber}"/>' 
												class="page-link">${pageNumber }</a></li>
											</c:if>
										</c:forEach>
											
										<li class="${userPage.totalPages == userPage.number+1? 'page-item active' : 'page-item' }">
										<a href='<c:url value="/admin/users/searchpagenated?name=${name }&size=${userPage.size}&page=${userPage.totalPages}"/>' class="page-link">Last</a>
										</li>
									</ul>
								</nav>
							</c:if>
					...
					<div class="col-md-6">
						<div class="float-right">
							<a class="btn btn-outline-success" href="/admin/users/add">Add New User</a>
						</div>
					</div>
				</div>
