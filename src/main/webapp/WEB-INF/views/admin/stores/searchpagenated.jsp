							...
							<tbody>
								<c:forEach items="${storePage.content}" var="store">
										...
										<tr>
											...
											<td>${store.open ? 'Mở cửa' : 'Đóng cửa'}</td>
							...
							<c:if test="${storePage.totalPages > 0 }">
								<nav aria-label="Page navigation">
									<ul class="pagination">
										...
										<a class="page-link"
											href='<c:url value="/admin/stores/searchpagenated?name=${name }&size=${storePage.size}&page=${1}"/>'
										...
										<c:forEach items="${pageNumbers }" var="pageNumber">
											<c:if test="${storePage.totalPages > 1 }">
												...
												<a href='<c:url value="/admin/stores/searchpagenated?name=${name }&size=${storePage.size}&page=${pageNumber}"/>' 
												class="page-link">${pageNumber }</a></li>
											</c:if>
										</c:forEach>
											
										<li class="${storePage.totalPages == storePage.number+1? 'page-item active' : 'page-item' }">
										<a href='<c:url value="/admin/stores/searchpagenated?name=${name }&size=${storePage.size}&page=${storePage.totalPages}"/>' class="page-link">Last</a>
										</li>
									</ul>
								</nav>
							</c:if>
